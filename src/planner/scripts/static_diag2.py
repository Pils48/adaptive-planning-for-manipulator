...
class PathPlanner:
    def __init__(self, h, w):
        ...
        map_shape = (h // min_discrete, w // min_discrete)
        self.map = clean_mark*np.ones(map_shape, np.int16)
        self.start = ()
        self.target = ()
        self.path = []
    ...
    def _get_path(self):
        ...
        while not ((j, i) == self.start):
            # left
            if i+1 < n and self.map[j, i+1] == step - 1:
                i += 1
            # right
            ...
            # up
                ...
                j += 1
            # down
            elif j-1 > 0 and self.map[j-1, i] == step - 1:
                j -= 1

            # diag (right-up)
            elif self.map[j-1, i+1] == step - 1:
                j -= 1
                i += 1
            # diag (right-down)
            elif self.map[j+1, i+1] == step - 1:
